...
contract MockHtsPrecompile is IHederaTokenService {
    ...
    int32 private _updateKeysResp = 22;
    int32 private _updateInfoResp = 22;
    ...
    address public lastUpdateInfoToken;
    address public lastUpdateInfoTreasury;
    ...
    function setUpdateKeysResponse(int32 code) external { _updateKeysResp = code; }
    function setUpdateInfoResponse(int32 code) external { _updateInfoResp = code; }
    ...
    // --- IHederaTokenService implementations ---

    function createFungibleToken(HederaToken memory, int64, uint8)
        external
        payable
        override
        returns (int32 responseCode, address tokenAddress)
    {
        ...
        if (_createTokenAddr != address(0)) {
            tokenAddress = _createTokenAddr;
        } else {
            _tokenCounter++;
            tokenAddress = address(uint160(uint256(keccak256(abi.encodePacked(_tokenCounter, block.timestamp)))));
        }
    }
        ...
        returns (int32 responseCode, int64 newTotalSupply, int64[] memory serialNumbers)
    {
        ...
        responseCode = _mintResp;
        newTotalSupply = 0;
        serialNumbers = new int64[](0); // FIX: Correctly initialize empty array
    }
    ...
    function cryptoTransfer(TransferList memory, TokenTransferList[] memory tokenTransfers)
        external
        override
        returns (int32 responseCode)
    {
    ...
    function updateTokenKeys(address, TokenKey[] memory)
        external
        override
        returns (int32 responseCode)
    {
        responseCode = _updateKeysResp;
    }

    // FIX: Implement the missing function from the interface
    function updateTokenInfo(address token, HederaToken memory tokenInfo)
        external
        override
        returns (int32 responseCode)
    {
        lastUpdateInfoToken = token;
        lastUpdateInfoTreasury = tokenInfo.treasury;
        responseCode = _updateInfoResp;
    }
}